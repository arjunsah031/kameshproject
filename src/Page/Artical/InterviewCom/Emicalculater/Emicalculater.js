...
export default function App() {
  ...
  return (
    <div className={CssStyle.App}>
      <span className={ CssStyle.title} style={{ fontSize: 30, marginTop: 10 }}>
        EMI Calculator
      </span>
      ...
      <span className={ CssStyle.title}>Tenure</span>
      <div className={CssStyle.tenureContainer}>
        {tenureData.map((t) => {
          return (
            <button
              className={` ${t === tenure ? CssStyle.selected : CssStyle.withoutselected}`}
              onClick={() => setTenure(t)}
            >
              {t}
            </button>
